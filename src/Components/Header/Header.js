...
const Header = ({ localeLogos, accountInfos, onGetDataSearch }) => {
    ...
    useLayoutEffect(() => {
        const handleShowHeader = () => {
            if (window.scrollY > 200 || document.documentElement.scrollTo > 200) {
                setFixedHeader(true)
            } else {
    ...
    useEffect(() => {
        ...
        // onGetDataSearch(results)
    }, [datas, onGetDataSearch, search])
