...
import OdenseCollectionFilter from "./OdenseCollectionFilter"

const OdenseCollection = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        ...
        const results = datas.filter((data, index) => {
            if (data.name.toLowerCase().includes('odense') || kitchenWare?.id === data['type_id']) {
                return data
            }
        })
        ...
        <main className="container product-lists">
            ...
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">ODENSE Collection</h2>
                    <OdenseCollectionFilter
                        dataKoges={dataKoges}
...
export default memo(OdenseCollection)