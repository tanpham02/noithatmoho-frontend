...
import FynCollectionFilter from "./FynCollectionFilter"

const FynCollection = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        const results = datas.filter((data, index) => {
            if (data.name.toLowerCase().includes('fyn')) {
                return data
            }
        })
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_fyn.png" alt="FYN Collection" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">FYN Collection</h2>
                    <FynCollectionFilter
                        dataKoges={dataKoges}
...
export default memo(FynCollection)