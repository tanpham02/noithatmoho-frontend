...
import SofaFilter from "./SofaFilter"

const Sofa = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        ...
        const results = datas.filter((data, index) => {
            const kitchenWare = dataTypes.find(type => type.id === 8)
            if (kitchenWare) {
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/ghe-sofa.png" alt="Ghế Sofa" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">Ghế Sofa</h2>
                    <SofaFilter
                        dataKoges={dataKoges}
