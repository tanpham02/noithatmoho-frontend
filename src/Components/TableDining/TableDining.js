...
import TableDiningFilter from "./TableDiningFilter"

const TableDining = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        ...
        const results = datas.filter((data, index) => {
            const kitchenWare = dataTypes.find(type => type.id === 12)
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/table-dining.png" alt="Bàn Ăn" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">Bàn Ăn</h2>
                    <TableDiningFilter
                        dataKoges={dataKoges}
...
export default memo(TableDining)