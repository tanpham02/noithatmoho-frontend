...
import SetDiningFilter from "./SetDiningFilter"

const SetDining = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        ...
        const results = datas.filter((data, index) => {
            const kitchenWare = dataTypes.find(type => type.id === 14)
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_set-dining.png" alt="Bộ Bàn Ăn" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">Bộ Bàn Ăn</h2>
                    <SetDiningFilter
                        dataKoges={dataKoges}
