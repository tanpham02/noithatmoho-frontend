...
import MilanCollectionFilter from "./MilanCollectionFilter"

const MilanCollection = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        const kitchenWare = dataTypes.find(type => type.id === 4)
        const results = datas.filter((data, index) => {
            if (data.name.toLowerCase().includes('milan') || kitchenWare?.id === data['type_id']) {
                return data
            }
        })
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_milan.png" alt="MILAN Collection" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">MILAN Collection</h2>
                    <MilanCollectionFilter
                        dataKoges={dataKoges}
...
export default memo(MilanCollection)