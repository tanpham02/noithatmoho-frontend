...
import MalagaCollecionFilter from "./MalagaCollecionFilter"

const MalagaCollecion = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        const results = datas.filter((data, index) => {
            if (data.name.toLowerCase().includes('malaga')) {
                return data
            }
        })
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_malaga.png" alt="MALAGA Collection" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">MALAGA Collection</h2>
                    <MalagaCollecionFilter
                        dataKoges={dataKoges}
...
export default memo(MalagaCollecion)