...
const DinningRoomFilter = ({ dataDinningRoom, onGetData, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        if (sort !== '') {
            const output = dataDinningRoom.sort((a, b) => {
                if (sort === 'Tăng Dần') {
...
export default memo(DinningRoomFilter)