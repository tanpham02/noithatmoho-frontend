...
const MohoWorkFilter = ({ dataMohoWork, onGetData, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        if (sort !== '') {
            const output = dataMohoWork.sort((a, b) => {
                if (sort === 'Tăng Dần') {
    ...
    onGetData(dataFilters.length ? dataFilters : dataMohoWork)
    return (
        <>
