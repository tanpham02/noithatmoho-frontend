...
const LivingRoomFilter = ({ dataLivingRoom, filterCategories, filterPrices, filterSort, onGetData }) => {
    ...
    useEffect(() => {
        if (sort !== '') {
            const output = dataLivingRoom.sort((a, b) => {
                if (sort === 'Tăng Dần') {
...
export default memo(LivingRoomFilter)