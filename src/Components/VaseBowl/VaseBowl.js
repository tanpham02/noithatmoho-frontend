...
import VaseBowlFilter from "./VaseBowlFilter"

const VaseBowl = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        ...
        const results = datas.filter((data, index) => {
            const kitchenWare = dataTypes.find(type => type.id === 23)
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/vase-bowl.png" alt="Bình - Chậu - Lọ" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">Bình - Chậu - Lọ</h2>
                    <VaseBowlFilter
                        dataKoges={dataKoges}
