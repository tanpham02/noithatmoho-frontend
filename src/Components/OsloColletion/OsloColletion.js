...
import OsloColletionFilter from "./OsloColletionFilter"

const OsloColletion = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        const results = datas.filter((data, index) => {
            if (data.name.toLowerCase().includes('oslo')) {
                return data
            }
        })
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_oslo.png" alt="OSLO Collection" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">OSLO Collection</h2>
                    <OsloColletionFilter
                        dataKoges={dataKoges}
...
export default memo(OsloColletion)