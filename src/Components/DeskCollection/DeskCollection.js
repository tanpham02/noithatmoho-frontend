...
import DeskCollectionFilter from "./DeskCollectionFilter"

const DeskCollection = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        const results = datas.filter((data, index) => {
            const kitchenWare = dataTypes.find(type => type.id === 20)
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_desk.png" alt="Bàn Làm Việc" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">Bàn Làm Việc</h2>
                    <DeskCollectionFilter
                        dataKoges={dataKoges}
...
export default memo(DeskCollection)