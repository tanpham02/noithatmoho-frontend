...
import NyborgCollectionFilter from "./NyborgCollectionFilter"

const NyborgCollection = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        const results = datas.filter((data, index) => {
            if (data.name.toLowerCase().includes('nyborg')) {
                return data
            }
        })
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_nyborg.png" alt="NYBORG Collection" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">NYBORG Collection</h2>
                    <NyborgCollectionFilter
                        dataKoges={dataKoges}
...
export default memo(NyborgCollection)