...
import KoldingCollectionFilter from "./KoldingCollectionFilter"

const KoldingCollection = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        const kitchenWare = dataTypes.find(type => type.id === 2)
        const results = datas.filter((data, index) => {
            if (data.name.toLowerCase().includes('kolding') || kitchenWare?.id === data['type_id']) {
                return data
            }
        })
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_kolding.png" alt="KOLDING Collection" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">KOLDING Collection</h2>
                    <KoldingCollectionFilter
                        dataKoges={dataKoges}
...
export default memo(KoldingCollection)