...
const MohoWorkFilter = ({ dataWorkRoom, onGetData, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        if (sort !== '') {
            const output = dataWorkRoom.sort((a, b) => {
                if (sort === 'Tăng Dần') {
    ...
    onGetData(dataFilters.length ? dataFilters : dataWorkRoom)
    return (
        <>
...
export default memo(MohoWorkFilter)