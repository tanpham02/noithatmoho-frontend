...
import MossCollectionFilter from "./MossCollectionFilter"

const MossCollection = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        const results = datas.filter((data, index) => {
            if (data.name.toLowerCase().includes('moss')) {
                return data
            }
        })
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_moss.png" alt="MOSS Collection" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">MOSS Collection</h2>
                    <MossCollectionFilter
                        dataKoges={dataKoges}
...
export default memo(MossCollection)