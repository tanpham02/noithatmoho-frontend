...
import KogeCollectionFilter from "./KogeCollectionFilter"

const KogeCollection = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        const results = datas.filter((data, index) => {
            if (data.name.toLowerCase().includes('koge')) {
                return data
            }
        })
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_koge.png" alt="KOGE Collection" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">KOGE Collection</h2>
                    <KogeCollectionFilter
                        dataKoges={dataKoges}
...
export default memo(KogeCollection)