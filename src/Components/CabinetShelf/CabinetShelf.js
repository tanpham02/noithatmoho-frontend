...
import CabinetShelfFilter from "./CabinetShelfFilter"

const CabinetShelf = ({ datas, dataTypes, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        ...
        const results = datas.filter((data, index) => {
            const kitchenWare = dataTypes.find(type => type.id === 22)
    ...
    return (
        <main className="container product-lists">
            <img className="img-page" src="/assets/img/img-page/collection_cabinet-shelf.png" alt="Tủ & Kệ" />
            <div className="grid">
                <div className="grid__row">
                    <h2 className="products__title px--16">Tủ & Kệ</h2>
                    <CabinetShelfFilter
                        dataKoges={dataKoges}
...
export default memo(CabinetShelf)