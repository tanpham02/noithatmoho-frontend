...
const DecorationFilter = ({ dataDecoration, onGetData, filterCategories, filterPrices, filterSort }) => {
    ...
    useEffect(() => {
        if (sort !== '') {
            const output = dataDecoration.sort((a, b) => {
                if (sort === 'Tăng Dần') {
    ...
    onGetData(dataFilters.length ? dataFilters : dataDecoration)
    return (
        <>
...
export default memo(DecorationFilter)